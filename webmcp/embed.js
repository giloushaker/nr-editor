(function () {
  /* nr-editor patch: under Electrons file:// the page has an opaque origin, so postMessage with it as targetOrigin is silently dropped. Fall back to "*" there; http(s) is untouched. */ function __wo() {
  ...
  function o(e) {
    if (i(e)) return e;
    let t = r(e);
    return { content: [{ type: `text`, text: a(e) }], ...(t === void 0 ? {} : { structuredContent: t }), isError: !1 };
  }
  function s(e) {
  ...
  function S(e) {
    if (e === null)
      return { isError: !0, content: [{ type: `text`, text: `Tool execution interrupted by navigation` }] };
    let t;
    try {
      t = JSON.parse(e);
    } catch {
...
`);
  ...
  function N() {
    ...
    T()
      .then((t) => {
        if (e !== O) return;
        let n = M(t);
        n === A || !d || ((A = n), d.postMessage({ type: `webmcp.tools.changed`, tools: t }, f.widgetOrigin));
      })
      .catch((e) => {
    ...
    let e = 0,
      ...
      n = () => {
        setTimeout(() => {
          if ((e++, !I())) {
            if (e >= 40) {
              g(`Could not subscribe to tool changes after 40 retries. Dynamic tool updates will rely on polling.`);
              return;
            }
  ...
  function R(e, t, n) {
    !e || typeof e != `object` || !(`postMessage` in e) || e.postMessage(n, __wo() === `*` ? `*` : t);
  }
  function z(e) {
  ...
  function B(e, t) {
    T()
      .then((n) => {
        R(t.source, t.origin, { type: `webmcp.tools.list.response`, requestId: e.requestId, tools: n });
      })
      .catch((n) => {
  ...
  function V(e, t) {
    E(String(e.toolName ?? ``), b(e.args))
      .then((n) => {
        R(t.source, t.origin, { type: `webmcp.tools.invoke.response`, requestId: e.requestId, result: s(n) ? n : {} });
      })
      .catch((n) => {
        R(t.source, t.origin, {
  ...
  async function H(e) {
    ...
    (t.set(`tabId`, e.tabId), t.set(`hostOrigin`, __wo()));
    ...
    let i = await r.text(),
      ...
      o = URL.createObjectURL(new Blob([i.replace(`</head>`, `${a}</head>`)], { type: `text/html` }));
    e.widgetOrigin = __wo();
  ...
  if (!document.querySelector(l)) {
    ...
    window.addEventListener(`message`, (e) => {
      if ((f.widgetOrigin !== `*` && e.origin !== f.widgetOrigin) || !d || e.source !== d) return;
      let t = e.data;
    ...
    (document.body ? e() : document.addEventListener(`DOMContentLoaded`, e, { once: !0 }),
      ...
      document.addEventListener(`visibilitychange`, () => {
        document.visibilityState === `visible` && d && d.postMessage({ type: `webmcp.connect` }, f.widgetOrigin);
      }));
  }
})();
